 ...
 * Orchestrates: translate → RAG detect → enrich → persist.
 * Also handles audio: transcribe → extract claims → detect each.
 */
...
export async function detectAudio(req: AudioDetectRequest): Promise<AudioDetectionResult> {
  ...
  // Detect in parallel batches of 3
  const detected: AudioDetectionResult["claims"] = [];
  for (let i = 0; i < claims.length; i += 3) {
    const batch = claims.slice(i, i + 3);
    const results = await Promise.all(
      batch.map((c) => detectClaim({ claim: c.claim, sourceContext: "audio/voice message", sessionId: req.sessionId, localise: req.localise }))
    );
